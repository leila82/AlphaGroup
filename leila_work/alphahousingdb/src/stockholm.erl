...
%%extract the Type of Data(apartment, room or villa).
    ...
    Type2=string:substr(Rilimit,TypeStart2,(TypeEnd2-TypeStart2)), 
...
%%extract the rent. 
  ...
  Data={"Stockholm",swedL(eliminate(Type2)),
       swedL(eliminate(Address)),swedL(eliminate(Room)),
       swedL(eliminate(Area))++" m",swedL(eliminate(Rent))++" Kr"},
Doc = {[	{<<"District">>, list_to_binary(
				 lists:append(swedL(eliminate(Address)), " / Stokholm"))},
				{<<"Rent">>, list_to_binary(swedL(eliminate(Rent)))},
				{<<"Rooms">>, list_to_binary(swedL(eliminate(Room)))},
				{<<"Area">>, list_to_binary(swedL(eliminate(Area)))}]},
...
case couchbeam_doc:is_saved(Doc) of
	...
	{NewDoc} = couchbeam_doc:set_value(<<"id">>, couchbeam_doc:get_id(Doc1), Doc),
  mochijson2:encode({struct, NewDoc})
...
%%fix the swedish letters

swedL(String)-> swedL(String,["Ã¥","Ã¤","Ã¶","Ã©","&amp;","Â´","","Ö¤","\r","Ã¤"],["å","ä","ö","é","&","","","ä","","ä"]).
swedL(String,[X|Xs],[Y|Ys])->
     NewString=re:replace(String, X,Y, [{return,list}]),
     swedL(NewString,Xs,Ys);
swedL(String,[],[]) ->String.
